...
export abstract class BaseAgent<T extends z.ZodType, M = unknown> {
  ...
  private setWithStructuredOutput(): boolean {
    ...
    // Ollama local models run much faster and reliably with manual JSON extraction (avoids CPU grammar constraint stalls)
    // Detect by provider string OR by the LangChain class name (ChatOllama) as fallback
    if (this.provider === ProviderTypeEnum.Ollama || this.chatModelLibrary === 'ChatOllama') {
      logger.debug(`[${this.modelName}] Ollama provider using manual JSON extraction for fast local inference`);
      return false;
    }
