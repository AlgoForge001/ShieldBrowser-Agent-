...
export function extractJsonFromModelOutput(content: string): Record<string, unknown> {
  try {
    ...
    // Fallback: try to extract JSON object substring from { to last }
    try {
      ...
      // continue to jsonrepair fallback
    }

    // Fallback: try jsonrepair for truncated / malformed output from small local models
    try {
