...
function createOpenAIChatModel(
  ...
  const effectiveMaxTokens = (modelConfig.parameters?.maxTokens as number) || 1024;
  ...
  if (isOpenAIReasoningModel(modelConfig.modelName)) {
    ...
    args.maxTokens = effectiveMaxTokens;
  }
  ...
  switch (modelConfig.provider) {
    ...
    case ProviderTypeEnum.Ollama: {
      const args: {
        ...
        maxTokens,
        // 4096 context window is sufficient for most prompts and halves CPU inference time vs 8192
        numCtx: 4096,
        // 1024 predict tokens — Navigator JSON output often exceeds 512 tokens causing truncation/parse failures
        numPredict: 1024,
      };
    ...
    case ProviderTypeEnum.OpenRouter: {
      ...
      return createOpenAIChatModel(providerConfig, modelConfig, {
        headers: {
          'HTTP-Referer': 'https://shieldbrowser.local',
          'X-Title': 'ShieldBrowser',
        },
      });
    }
